...
def check_neighbours(grid, x, y, z, w):
        ...
        for y_new in range(y - 1, y + 2):
            for z_new in range(z - 1, z + 2):
                for w_new in range(w - 1, w + 2):
                    if x_new == x and y_new == y and z_new == z and w_new == w:
                        if find_cube(grid, x_new, y_new, z_new, w_new):
                            active = 1
                    elif find_cube(grid, x_new, y_new, z_new, w_new):
                        active_count += 1
...
def main():
    ...
    try:
        with open(args.input, 'rt') as file:
            ...
            for line in file:
                ...
                for char in line:
                    if char == '#':
                        active_cubes.append({'x': x, 'y': y, 'z': z, 'w': 0})
                    x += 1
                y += 1
    ...
    while count < total_cycles:
            ...
            for z in range(min_z - 1, max_z + 2):
                for y in range(min_y - 1, max_y + 2):
                    for x in range(min_x - 1, max_x + 2):
                        #print("{} {} {}".format(x, y, z))
                        if check_neighbours(active_cubes, x, y, z, w) == 1:
                            active_cubes_new.append({'x': x, 'y': y, 'z': z, 'w': w})
        ...
        #pprint(active_cubes)
        print("cycle {} took {} s".format(count, datetime.now() - start_time))
        print("Grid size x[{}->{}] y[{}->{}] z[{}->{}] w[{}->{}]".format(min_x, max_x, min_y, max_y, min_z, max_z, min_w, max_w))
