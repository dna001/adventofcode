...
def find_pos(map, char):
    """Find position based on char."""
    for y in range(len(map)):
        for x in range(len(map[0])):
            if map[y][x] == char:
                return {'x': x, 'y': y, 'cost': 1}
    return None
...
def main():
    ...
    cost = pathfinder.find_path(nodes[-1], nodes[0])

    race_path_nodes = []
    for node in nodes:
        if 'pos' in node:
            new_node = {'x' : node['x'], 'y': node['y'], 'pos': node['pos']}
            race_path_nodes.append(new_node)
    end_node = find_pos(map, 'E')
    # Add end node since it's skipped in pathfinding code
    race_path_nodes.append({'x' : end_node['x'], 'y': end_node['y'], 'pos': cost})

    print(f"Cost: {cost}, {len(race_path_nodes)}")
    #print(race_path_nodes)

    if args.second:
        # Sort nodes by position
        race_path_nodes.sort(key=myFunc)
        print(race_path_nodes)
        # Find cheats up to 20 picoseconds long using manhattan distance
        cheat_nodes = {}
        for i in range(0, len(race_path_nodes) - 100):
            for j in range(i + 100, len(race_path_nodes)):
                x_diff = abs(race_path_nodes[i]['x'] - race_path_nodes[j]['x'])
                y_diff = abs(race_path_nodes[i]['y'] - race_path_nodes[j]['y'])
                cheat_dist = x_diff + y_diff
                pos_diff = race_path_nodes[j]['pos'] - race_path_nodes[i]['pos']
                diff = pos_diff - cheat_dist
                if diff >= 100 and cheat_dist <= 20:
                    if str(diff) in cheat_nodes:
                        cheat_nodes[str(diff)] += 1
                    else:
                        cheat_nodes[str(diff)] = 1
        print(cheat_nodes)
        n_cheats = 0
        for key, value in cheat_nodes.items():
            if int(key) >= 100:
                n_cheats += value

        print(f"Cheats that save 100ps or more: {n_cheats}")
    else:
        cheat_nodes = {}
        cheat_diff = 0
        # Find best cheat
        for node in race_path_nodes:
            for coord in [[-2, 0], [2, 0], [0, -2], [0, 2]]:
                cheat_node = find_node(race_path_nodes, node['x'] + coord[0], node['y'] + coord[1])
                if cheat_node != None and cheat_node['pos'] > node['pos']:
                    diff = cheat_node['pos'] - node['pos'] - 2
                    if diff > 0:
                        if diff > cheat_diff:
                            cheat_diff = diff
                        if str(diff) in cheat_nodes:
